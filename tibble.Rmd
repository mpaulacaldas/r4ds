...
1.  How can you tell if an object is a tibble? (Hint: try printing `mtcars`,
    ...
    ::: answer
    Tibbles have a nicer printing method, which limits the number of rows displayed and returns the column types.
    ```{r}
    mtcars
...
1.  Compare and contrast the following operations on a `data.frame` and 
    ...
    Column subsetting always returns a tibble:
...
1.  If you have the name of a variable stored in an object, e.g. `var <- "mpg"`,
    ...
    If I want my output to be a tibble, I can do:
    ```{r}
    tb_mtcars[, var]
    tb_mtcars[var]
    ```
    
    Otherwise, to extract the vector, I would do:
...
1.  What does `tibble::enframe()` do? When might you use it?
    ...
    ::: answer
    `enframe()` can be used to convert a named character string or list in to a tibble. Note that each element gets its own row.
    ```{r}
    (vec <- c(a = 5, b = 7))
    enframe(vec)
    
    (ls <- list(one = 1, two = 2:3, three = 4:6))
    enframe(ls)
    ```
    :::
