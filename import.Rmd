...
1.  What are the most important arguments to `read_fwf()`?
    ...
    <div class="answer">
    The most important argument is `col_positions`, which must be created using the helper functions `fwf_empty()`, `fwf_widths()` or `fwf_positions()`.
    </div>
...
1.  Identify what is wrong with each of the following inline CSV files. 
    ...
    <div class="answer">
    There is a missing column header.
    ```{r}
    read_csv("a,b\n1,2,3\n4,5,6")
    ```

    There is a missing column in the fist row, and an extra one in the second.
    ```{r}
    read_csv("a,b,c\n1,2\n1,2,3,4")
    ```
    
    There is a missing closing quote.
    ```{r}
    read_csv("a,b\n\"1")
    ```
    
    It looks like the column names are repeated at the end of the file, which coerces the columns to be imported with character type.
    ```{r}
    read_csv("a,b\n1,2\na,b")
    ```
    
    Wrong separator, need to use `read_csv2()`.
    ```{r}
    read_csv("a;b\n1;3")
    read_csv2("a;b\n1;3")
    ```
...
1.  What are the most important arguments to `locale()`? 
    ...
    <div class="answer">
    From experience, the most useful ones are `grouping_mark`, `decimal_mark` and `encoding`. 
    </div>
...
1.  What's the difference between `read_csv()` and `read_csv2()`?
    ...
    <div class="answer">
    `read_csv()` uses colons (`,`) as separators, whereas `read_csv2()` uses semi-colons (`;`).
    </div>
...
*   If you're having major parsing problems, sometimes it's easier
    ...
    <div class="answer">
    `r emo::ji("exploding_head")` How did I not know about `read_file()`?
    </div>
