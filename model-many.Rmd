...
1.  A linear trend seems to be slightly too simple for the overall trend.
    ...
    ::: answer
    Below I specify the new model, and go over the previous preparation steps. There are two main differences: I center the `year` variable around the mean within each group, and I keep only the `resid` variable generated by `add_residuals()` to prevent generating duplicate columns when unnesting.
    ```{r, error = TRUE}
    country_model2 <- function(data) {
      lm(lifeExp ~ year + I(year^2), data = data)
    }
    ...
    by_country2 <- gapminder %>% 
      ...
      ungroup() %>% 
      mutate(
        model = map(data, country_model2),
        resids = map2(data, model, add_residuals) %>% map("resid"),
        glance = map(model, broom::glance)
        ) %>% 
      unnest(c(data, resids, glance))
    
    by_country2
    ```
    
    Our new model seems to do better than the previous one, particularly in explaining the increase in life expectancy between 1980 and 1990.
    
    ```{r, ref.label="ggplot-resid"}
    ```
    
    ```{r}
    ggplot(by_country2, aes(year, resids)) +
      geom_line(aes(group = country), alpha = 1 / 3) +
      geom_smooth(se = FALSE) +
      labs(title = "New model: quadratic polynomial")
    ```
    
    There remain however extreme trends in Africa and Asia which are not well-explained by the model.
    
    ```{r, ref.label = "ggplot-resid-continent"}
    ```
    
    ```{r}
    ggplot(by_country2, aes(year, resids, group = country)) +
      geom_line(alpha = 1 / 3) +
      facet_wrap(~ continent) +
      labs(title = "New model: quadratic polynomial")
    ```
    
    Notably, there remains only one country in Africa with an $R^2$ below 0.25.
    
    ```{r, ref.label="ggplot-resid-jitter"}
    ```
    
    ```{r}
    by_country2 %>% 
      distinct(continent, r.squared) %>% 
      ggplot(aes(continent, r.squared)) +
      geom_jitter(width = 0.5) +
      labs(title = "New model: quadratic polynomial")
...
1.  Explore other methods for visualising the distribution of $R^2$ per
    ...
    ::: answer
    Let's first set up the basic architecture of our plots:
    ```{r}
    p_country_r2 <- by_country2 %>% 
      distinct(continent, r.squared) %>% 
      ggplot(aes(continent, r.squared))
    ```
    
    One way to visualise the $R^2$ is to use boxplots. Although the outliers for each continent are clearly visible, we lose some information about the distribution of points. For one, we have no way of knowing if the groups are more or less equivalent in size.
    ```{r}
    p_country_r2 +
      geom_boxplot()
    ```
    
    Another possible visualisation is violin plots, which displays a mirror density plot. They give us slightly more information about the distribution within each group, but do badly when it comes to displaying outliers or small samples.
    ```{r}
    p_country_r2 +
      geom_violin()
    ```
    
    Finally, beeswarm (or violin) scatter plots do a better job of visualising the distribution of points within each continent while keeping each observation visible.
    ```{r}
    library(ggbeeswarm)
    p_country_r2 +
      geom_beeswarm()
...
1.  To create the last plot (showing the data for the countries with the
    ...
    ::: answer
    I already did something similar in the first question. The trick is to unnest both the `resids` and `glance` list-columns. `resids` contains both the original columns from the `data` list-column plus a `resid` column with the residuals per observation. `glance` has all the model diagnostics, including `r.squared`.
    
    Below is the new pipeline. Note that I still get rid of the extra list-columns, but this time I do so via `select_if()`.
    ```{r}
    gapminder %>% 
      group_by(continent, country) %>% 
      nest() %>% 
...
1.  List all the functions that you can think of that take a atomic vector and 
    ...
    ::: answer
    Some examples from **stringr**:
    ```{r}
    lett_num <- c("a1b2", "c3d4ef")
    
    str_extract_all(lett_num, "[a-z]+")
    str_split(lett_num, "[a-z]")
    str_match_all(lett_num, "\\d")
    ```
...
1.  What's missing in the following data frame? How does `quantile()` return
    ...
    ::: answer
    The missing piece is the probabilities, which are generally returned in the list names.
    ```{r}
    mtcars %>% 
      filter(cyl == 4) %>% 
      pull(mpg) %>% 
    ...
    This behaviour is unhelpful when working with list-columns because there it makes it harder to put the information into a separate column. A work-around could be:
    
    ```{r}
    quantile_tbl <- function(...) {
      tibble::tibble(
        quantile = quantile(...),
        proba = names(quantile)
      )
    }
    
    mtcars %>% 
      group_by(cyl) %>% 
      summarise(q = list(quantile_tbl(mpg))) %>% 
      unnest()
    ```
...
1.  What does this code do? Why might might it be useful?
    ...
    ::: answer
    It turns all columns except `cyl` into list columns. It may be useful when we want to do complex operations that require lists, or if we want to make row-wise operations.
    ```{r, ref.label="summarise-all", eval = TRUE}
    ```
    :::
...
1.  Why might the `lengths()` function be useful for creating atomic
    ...
    ::: answer
    `lengths()` returns the length of each element of a list. It is helpful if we want to check that each of the elements of our list-column have the same length before we attempt to unnest.
...
1.  List the most common types of vector found in a data frame. What makes
    ...
    ::: answer
    The most common types of vectors found in a data frame are numeric (either integer or double), logical, character and factor. Regular data frame columns require vectors to be atomic and of the same length. List-columns only require that the *list* be of the same length as the other vectors, but there are no conditions on the types of vectors that can be stored *inside* the list.
    :::
