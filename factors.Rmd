...
1.  Explore the distribution of `rincome` (reported income). What makes the
    ...
    The labels overlap in the default chart. 
    
    ```{r}
    (p_income <- ggplot(gss_cat, aes(rincome)) +
      geom_bar())
    ```
    
    One way to improve it would be to flip the axes.
...
1.  What is the most common `relig` in this survey? What's the most
    ...
    ```{r}
    gss_cat %>% 
      count(relig, sort = TRUE) %>% # sort when counting, take 1st
      slice(1)
    gss_cat %>% 
      count(partyid) %>% # count, take the top 1
      top_n(1)
    ```
...
1.  Which `relig` does `denom` (denomination) apply to? How can you find
    ...
    At first glance, we can see that `denom` most likely applies to the `"Protestant"` because `r pairs$n[1]` levels of the `denom` factor are directly associated to `relig == "Protestant"`.
    
    ```{r, ref.label="pairs", results="show"}
    ```
    
    We can confirm this first impression by looking at the factor levels of `denom` that are associated to other religions. As we see below, other religions are labelled as `"Not applicable"`, `"No answer"`, `"No denomination"` or `"Don't know"`.
    
    ```{r}
    pairs %>% 
      filter(n < max(n)) %>% 
      semi_join(gss_cat, ., by = "relig") %>% 
      distinct(relig, denom) %>% 
      count(denom, sort = TRUE)
    ```
    
    A quick way to visualise these relationships is by using dots to represent the all possible `relig`-`denom` pairs.
...
<div class="answer">
This could be driven by elderly adults who do not perceive income.
</div>
...
1.  There are some suspiciously high numbers in `tvhours`. Is the mean a good
    ...
    No, it would be best to take the median. 
...
1.  For each factor in `gss_cat` identify whether the order of the levels is
    ...
    Let's first have a look at the levels:
    ...
    |`marital` | Arbitrary  |
    |`race`    | Arbitrary  |
    |`rincome` | Principled, because income levels are ordered |
    |`partyid` | Principled, because political affiliation is ordered |
    |`relig`   | Arbitrary  |
    |`denom`   | Arbitrary  |
...
1.  Why did moving "Not applicable" to the front of the levels move it to the
    ...
    All factor levels have an associated integer value, which is what allows us to give them an order. When we apply `fct_relevel(rincome, "Not applicable")`, the level `"Not applicable"` becomes the factor level with the lowest associated value, as we can see below.
    
    ```{r}
    gss_cat %>% 
      select(rincome) %>% 
      mutate(
        rincome_val = as.numeric(rincome),
        rincome_releved = fct_relevel(rincome, "Not applicable"),
        rincome_releved_val = as.numeric(rincome_releved)
        )
    ```
    
    ggplot2 uses these underlying values to arrange our factors in the x or y axis. Factors with lower underlying values will be closer to the lower-left corner of the plot, which corresponds to the (0,0) point in the (x, y) coordinate plane. 
...
1.  How have the proportions of people identifying as Democrat, Republican, and
    ...
    props %>% 
      ggplot(aes(year, prop, colour = fct_reorder(partyid, -prop))) +
      geom_line()
    ```
    </div>
...
1.  How could you collapse `rincome` into a small set of categories?
    ...
    Similarly to my previous answer, I would manually define broader income brackets using `fct_collapse()`, and would lump all of the other factor levels by setting the `group_other` argument to `TRUE`. 
