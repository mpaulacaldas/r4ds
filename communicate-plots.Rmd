...
1.  Create one plot on the fuel economy data with customised `title`,
    ...
    ggplot(mpg, aes(displ, hwy, colour = class)) +
      ...
      facet_wrap(~ class) +
      labs(
       title =  "Fuel efficiency tends to decrese with engine size",
       subtitle = "The relationship is less clear for two seaters and mini vans",
       x = "Engine displacement (L)",
       y = "Highway fuel economy (mpg)",
       caption = "Data from fueleconomy.org",
       colour = "Car type"
      )
    ```
    </div>
...
1.  The `geom_smooth()` is somewhat misleading because the `hwy` for
    ...
    <div class="answer">
    The first thing I want to do is to compare the predictions given by the `loess` method before with the predictions of a simple linear trend. My guess is that the polynomial approach of `loess` is more sensitive to extreme values.
    
    ```{r}
    ggplot(mpg, aes(displ, hwy)) +
      geom_point(aes(colour = class)) +
      geom_smooth(se = FALSE, method = "loess") +
      geom_smooth(se = FALSE, method = "lm", colour = "hotpink") +
      labs(
        title = "A linear trend is less biased by two seaters",
        x = "Engine displacement (L)",
        y = "Highway fuel economy (mpg)",
        colour = "Car type"
      )
    ...
    I now fit a linear model for each car type to see if the negative relationship between fuel efficiency and engine size is similar across groups.
    ...
    ggplot(mpg, aes(displ, hwy, colour = class)) +
      ...
      facet_wrap(~ class) +
      labs(
       title =  "The linear trend is negative within each class",
       subtitle = "The relationship is less pronounced for two seaters and mini vans",
       x = "Engine displacement (L)",
       y = "Highway fuel economy (mpg)",
       colour = "Car type"
      )
    ...
    With this information in mind, let's fit a linear model on the entire data set, controlling for the car type. The resulting predictions are less pronounced than the trends given by the models fitted within each class, but provide a better fit than the first linear model that did not control for car type.
    
    ```{r, fig.asp = 1}
    model_mpg <- lm(hwy ~ displ + factor(class), data = mpg)
    
    mpg %>% 
      modelr::add_predictions(model_mpg) %>% 
      ggplot(aes(displ, hwy)) +
      geom_point(aes(color = class), alpha = 1/3) +
      geom_smooth(se = FALSE, method = "lm", colour = "red") +
      geom_line(aes(y = pred, color = class), size = 1) +
      facet_wrap(~class) +
      labs(
       title =  "The FE predictions provide a better fit than a simple linear model",
       x = "Engine displacement (L)",
       y = "Highway fuel economy (mpg)",
       colour = "Car type"
      )
...
<div class="answer">
I am curious about the arguments passed to `ggrepel::geom_label_repel()`.

- `size = 6` increases the size of the text.
  ```{r}
  ggplot(mpg, aes(displ, hwy, colour = class)) +
    ggrepel::geom_label_repel(aes(label = class),
      data = class_avg,
...
- `label.size = 0` reduces the size of the label border to 0mm.
  ```{r}
  ggplot(mpg, aes(displ, hwy, colour = class)) +
    ggrepel::geom_label_repel(aes(label = class),
      data = class_avg,
...
- `segment.color = NA` removes the colour from the line segment conecting the data point to the label. The change here is imperceptible.
  ```{r}
  ggplot(mpg, aes(displ, hwy, colour = class)) +
    ggrepel::geom_label_repel(aes(label = class),
      data = class_avg,
...
1.  Use `geom_text()` with infinite positions to place text at the
    ...
    label_infs <- tibble(
      ...
      vjust = c("top", "bottom", "bottom", "top"),
      hjust = c("right", "right", "left", "left"),
      label = paste(vjust, hjust, sep = "-")
    )
...
1.  Read the documentation for `annotate()`. How can you use it to add a text
    ...
    <div class="answer">
    You can specify the coordinates directly in the `x` and `y` arguments of `annotate()`.
    ```{r}
    ggplot(mpg, aes(displ, hwy)) +
      geom_point() +
      annotate(
        "text", # type of annotation we want
        x = c(4, 6),
        y = 35, # recycled if shorter length
        label = c("One annotation", "And another one")
        )
    ```
    </div>
...
1.  How do labels with `geom_text()` interact with faceting? How can you
    ...
    <div class="answer">
    To enable facetting, we have to add the `class` variable to the data frame passed to `geom_text()`.
    ```{r, fig.asp = 1}
    label_facets <- tibble(
        displ = 5,
        hwy = 40,
...
1.  What arguments to `geom_label()` control the appearance of the background
    ...
    <div class="answer">
    The `label.*` arguments control the appearance of the background box.
    - `label.padding`: padding around the label.
    - `label.r`: radius of rounded corners.
    - `label.size`: size of label border.
    
    Note that the values of `label.padding` and `label.r` need to be defined via `grid::unit()`.
    
    ```{r}
    mpg_small <- mpg %>% 
      filter(class %in% c("subcompact", "2seater"))
    
    best_in_class_small <- mpg_small %>% 
      group_by(class) %>% 
      filter(row_number(desc(hwy)) == 1)
    
    skeleton <- ggplot(mpg_small, aes(displ, hwy)) +
      geom_point(aes(colour = class)) +
      theme(legend.position = "none")
      
    skeleton +
      geom_label(
        data = best_in_class_small,
        aes(label = model)
        ) +
      labs(title = "Default")
    ...
    skeleton +
      geom_label(
        data = best_in_class_small,
        aes(label = model),
        label.padding = unit(0.5, "lines"), # double the default
        label.r = unit(0.45, "lines"), # thrice the default
        label.size = 0.5 # double the default
        ) +
      labs(
        title = "Increase padding, 'roundness' of corners and size of the border"
        )
    ```
    </div>
...
1.  What are the four arguments to `arrow()`? How do they work? Create a series
    ...
    <div class="answer">
    The arguments to `grid::arrow()` are:
    - `angle`: controls the "pointiness" of the arrow head, in degrees.
    - `length`: specifies the length of the arrow head, and needs to be defined via `grid::units()`.
    - `ends`: indicates if an arrow head should be drawn at the `"last"`, `"first"` or `"both"` ends of the arrow.
    - `type`: whether the arrow head is an `"open"` or `"closed"` triangle.
    
    Here is an example using `geom_segment()` showing these options:
    ...
    ggplot(df, aes(x, y, xend = xend, yend = yend)) +
      geom_segment(
        arrow = arrow(
          angle = c(10, 20, 30),
          length = unit(c(1.5, 1, 0.5), "cm"),
          ends = c("last", "first", "both"),
          type = c("open", "closed", "open")
          )
        )
    ```
    
    Here is another example using `geom_curve()` with some actual data.
    
    ```{r}
    skeleton +
      geom_label(data = best_in_class_small, aes(label = model)) +
      geom_curve(
        aes(x = 5.7, y = 27, xend = 2.5, yend = 44),
...
1.  Why doesn't the following code override the default scale?
    ...
    ```{r scale-gradient, fig.show = "hide"}
    ggplot(df, aes(x, y)) +
      geom_hex() +
    ...
    ```{r, ref.label="scale-gradient", fig.show = "hold", echo = FALSE}
    ```
    
    The code doesn't work as expected because it mapped the scale gradient to the wrong aesthetic, `colour`, instead of `fill`.
...
1.  What is the first argument to every scale? How does it compare to `labs()`?
    ...
    <div class="answer">
    The first argument to the scale functions is `name`, the name of the scale. `name` is set to `waiver()` by default, which takes the name from the first mapping used for that aesthetic. This argument is useful if we want to change or remove the default name of the scale.
    ...
    `waiver()` is also the default of all the arguments in `labs()`.
    
    Note that the first argument to `labs()` is actually `...`, which takes a list of name-value pairs, where the names need to be an aesthetic. Similar to the scale functions, you can chose to overrite the default name or pass `NULL` to remove the label.
...
1.  Change the display of the presidential terms by:
    ...
    ggplot(presidential2, aes(start, id, colour = party)) +
      ...
      scale_y_continuous(
        ...
        breaks = min(presidential2$id):max(presidential2$id),
        minor_breaks = NULL
        ) +
      scale_colour_manual(
        NULL,
      ...
      labs(
        title = "Terms of 11 US presidents",
