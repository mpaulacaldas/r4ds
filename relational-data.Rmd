...
1.  Imagine you wanted to draw (approximately) the route each plane flies from
    ...
    <div class="answer">
    I would need the latitude and longitude of the origin and destination airports. To get this information, I would need to combine the `flights` table with `airport`.
    </div>
...
1.  I forgot to draw the relationship between `weather` and `airports`.
    ...
    <div class="answer">
    `weather` and `airports` are related by the airport code, identified by the `origin` and `faa` variables.
    </div>
...
1.  `weather` only contains information for the origin (NYC) airports. If
    ...
    <div class="answer">
    The additional relation would be defined by the `dest` variable, which indicates the airport code of the destination airport.
    </div>
...
1.  We know that some days of the year are "special", and fewer people than
    ...
    <div class="answer">
    The primary key of such a table should be the date. The date can be expressed with the `month`, `day` and `hour` variables for consistency with the `weather` and `flights` tables.
    </div>
...
1.  Identify the keys in the following datasets
    ...
    1.  `Lahman::Batting`,
         ...
         ```{r}
         Lahman::Batting %>% 
           count(playerID, yearID, stint, sort = TRUE)
         ```
         </div>
    ...
    1.  `nasaweather::atmos`
        ...
        ```{r}
        nasaweather::atmos %>% 
          count(lat, long, year, month, sort = TRUE)
        ```
        </div>
    ...
    1.  `ggplot2::diamonds`
        <div class="answer">
        There is no primary key, and there are duplicates.
        ```{r}
        ggplot2::diamonds %>% 
          group_by_all() %>% 
          summarise(n = n()) %>% 
...
1.  Compute the average delay by destination, then join on the `airports`
    ...
    flights %>% 
      ...
      filter(lon > -140) %>% # focus on continental US
      ggplot(aes(lon, lat, colour = arr_delay)) +
      borders("state") +
      geom_point() + 
      coord_quickmap() +
...
1.  Add the location of the origin _and_ destination (i.e. the `lat` and `lon`)
    ...
    flights %>% 
      ...
      left_join(
        airports, 
        by = c("dest" = "faa"), 
        suffix = c("_origin", "_dest") # nicer column names
        )
    ```
    </div>
...
1.  Is there a relationship between the age of a plane and its delays?
    ...
    <div class="answer">
    No, there seems to be no relationship between the age of the plane and its delays.
    ```{r, fig.align = "default", out.width = "50%"}
    (flights_delay <- flights %>% 
      group_by(tailnum, year) %>% 
      summarise_at(vars(dep_delay, arr_delay), mean, na.rm = TRUE) %>% 
      ungroup() %>% 
    ...
    (age_delay <- planes %>% 
      ...
      inner_join(flights_delay, by = "tailnum") %>% 
...
1.  What weather conditions make it more likely to see a delay?
    ...
    <div class="answer">
    It's hard to tell. There seems to be a slightly positive relationship between `dep_delay` and `visibility`, `dewp` and `humidity`.
    ```{r, fig.asp = 1}
    flights %>% 
      ...
      filter(!is.na(value), !is.na(dep_delay)) %>% 
      ggplot(aes(dep_delay, value)) +
...
1.  What happened on June 13 2013? Display the spatial pattern of delays,
    ...
    ```{r}
    june1303 <- flights %>% 
      filter(year == 2013, month == 6, !is.na(arr_delay)) %>% 
      group_by(dest) %>% 
...
1.  What does it mean for a flight to have a missing `tailnum`? What do the
    ...
    <div class="answer">
    The flights with a missing `tailnum` also don't have information on the departure time, arrival time, air time or delay. These are probably flights that were cancelled.
    ```{r}
    flights %>% 
    ...
    It seems like a disproportionate number of these flights belong to the `"MQ"` and `"AA"` carriers.
    ```{r}
    flights %>% 
      anti_join(planes, by = "tailnum") %>%
...
1.  Find the 48 hours (over the course of the whole year) that have the worst
    ...
    <div class="answer">
    I didn't really get this exercise.
    
    Anyways, here is a possible approach. I calculate the average departure delay per day. Then I average that again with the average of the day before (I know, fishy). I isolate the days where this indicator is the highest, and flag them in `weather`. Finally, I create boxplots comparing the distributions of the main variables in `weather` for the flagged and *unflagged* (is it a word?) days.
    ```{r}
    worst_two_days <- flights %>% 
      group_by(year, month, day) %>% 
      summarise(dep_delay = mean(dep_delay, na.rm = TRUE)) %>% 
      ungroup() %>% 
      mutate(rolling_dep_delay = (dep_delay + lag(dep_delay)) / 2) %>% 
      filter(rolling_dep_delay == max(rolling_dep_delay, na.rm = TRUE)) %>% 
      mutate(day = list(c(day - 1, day))) %>% 
      unnest(day) %>% 
      mutate(worst_two = TRUE)
    
    worst_two_days
      
    weather %>% 
      left_join(worst_two_days, by = c("year", "month", "day")) %>% 
      mutate_at(vars(worst_two), coalesce, FALSE) %>% 
      pivot_longer(temp:visib) %>% 
      ggplot(aes(name, value, color = worst_two)) +
      geom_boxplot()
    ```
    </div>
...
1.  What does `anti_join(flights, airports, by = c("dest" = "faa"))` tell you?
    ...
    <div class="answer">
    `anti_join(flights, airports, by = c("dest" = "faa"))` returns information on the flights going to airports for which we have no information. 
    
    `anti_join(airports, flights, by = c("faa" = "dest"))` returns information on the airports that are not listed as destinations in the `flights` table.
    </div>
...
1.  You might expect that there's an implicit relationship between plane
    ...
    <div class="answer">
    A `tailnum` can be associated to more than one `carrier`.
    ```{r}
    ...
    These are the airplanes that are shared by more than one airline:
    ```{r}
    tailnum_carrier %>% 
      count(tailnum) %>% 
      filter(n > 1) %>% 
    ...
    It seems like the `tailnum` is not a unique identifier of a plane.
    
    ```{r}
    planes %>% 
      count(tailnum, year, sort = TRUE)
    ```
    
    Therefore, airlines may not actually be "sharing" planes. Most likely, they have bought different yearly models of the same type of airplane. Here is a list of the airlines that share `tailnum` information:
    ```{r}
    tailnum_carrier %>% 
      group_by(tailnum) %>% 
      filter(n() > 1) %>% 
