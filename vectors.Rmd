...
1.  Describe the difference between `is.finite(x)` and  `!is.infinite(x)`.
    ...
    <div class="answer">
    `is.finite(x)` will not return `TRUE` for `NA` or `NaN` values, but `!is.infinite(x)` will.
    
    ```{r}
    x <- c(-Inf, NaN, Inf, NA, 1)
    is.finite(x)
    !is.infinite(x)
    ```
...
1.  Read the source code for `dplyr::near()` (Hint: to see the source code,
    ...
    <div class="answer">
    The source code of `dplyr::near()` is:
    
    ```{r}
    dplyr::near
    ```
    
    The function calculates the absolute difference between `x` and `y` and checks if the resulting value is less than the square root of the smallest positive floating-point number in our machine.
...
1.  A logical vector can take 3 possible values. How many possible
    ...
    The maximum range of representable integers in R is $\pm2^{31}−1$, or `r scales::comma(((2^31) - 1) * 2)`. This is because the current implementations of R uses 32-bit integers for integer vectors (see `?integer`). 
    
    We can access the maximum integer value, $2^{31}−1$, via:
    ...
    The precision range of absolute values in R varies from around *2e-308* to *2e+308* (see `?double`). These exact values can be accessed via:
    ...
    *For  a more comprehensive answer, see <https://jrnold.github.io/r4ds-exercise-solutions/vectors.html#exercise-20.3.3>.*
...
1.  Brainstorm at least four functions that allow you to convert a double to an
    ...
    as.integer(x) # coercion to integer, rounds towards zero
...
<div class="answer">
`r emo::ji("exploding_head")` `rep()` has an `each` parameter!

Here is another useful one:
```{r}
tibble(x = 1:4, y = rep(1:2, length.out = length(x)))
```
...
1.  What does `mean(is.na(x))` tell you about a vector `x`? What about
    ...
    <div class="answer">
    `mean(is.na(x))` returns the proportion of missing values in `x`, and `sum(!is.finite(x))` the number of elements of `x` that are not finite (which can include missing values).
...
1.  Carefully read the documentation of `is.vector()`. What does it actually
    ...
    <div class="answer">
    According to the documentation, *`is.vector()` returns `TRUE` if `x` is a vector of the specified mode having no attributes other than names.* For example, `is.vector()` of a date will return `FALSE` because, despite being a vector of type numeric, one of its attributes is the class `"Date"`.
        
    ```{r}
    is.vector(c(a = 1, b = 2))
    attributes(c(a = 1, b = 2))
    
    is.vector(lubridate::today())
    attributes(lubridate::today())
    ```
    
    `is.atomic()` checks whether an object is of an atomic *type* ("logical", "integer", "numeric", "complex", "character" and "raw") or `NULL`. The condition is thus more general than `is.vector()`, which imposes restrictions on the object's attributes. 
    
    ```{r}
    is.atomic(lubridate::today())
    typeof(lubridate::today())
    ```
...
1.  Compare and contrast `setNames()` with `purrr::set_names()`.
    ...
    `stats::setNames()` is a convenience function that sets the names of an object and returns the object, which makes it pipe-friendly:
    
    ```{r}
    my_names <- c("one", "two", "three")
    ...
    `purrr::set_names()` expands the functionality of `stats::setNames()`, namely:
    
    - *If you do nothing, `x` will be named with itself.*
      ```{r}
      (one_two <- list("one", "two"))
      purrr::set_names(one_two)
      ```
    - *If `x` already has names, you can provide a function or formula to transform the existing names. In that case, `...` is passed to the function.*
      ```{r}
      (uno_dos <- list(un_o = 1, do_s = 2))
      purrr::set_names(uno_dos, stringr::str_remove, "_")
      ```
    - *If `nm` is `NULL`, the names are removed (if present).*
      ```{r}
      (un_deux <- list(un = 1, deux = 2))
      
      purrr::set_names(un_deux, NULL)
      ```
...
1.  Create functions that take a vector as input and returns:
    ...
    1. The last value.  Should you use `[` or `[[`?
        ...
        <div class="answer">
        Both work, but `[[` is better practice because it let's programmers know that we are expecting only one return value.
        ```{r}
        take_last1 <- function(x) { x[length(x)] }
        take_last2 <- function(x) { x[[length(x)]] }
    ...
    1. The elements at even numbered positions.
        ...
        <div class="answer">
        ```{r}
        get_from_even <- function(x) {
          pos <- (seq_along(x) %% 2) == 0
          x[pos]
        }
        
        get_from_even(letters)
        ```
        </div>
    ...
    1. Every element except the last value.
        ...
        <div class="answer">
        ```{r}
        all_but_last <- function(x) { x[-length(x)] }
...
1.  Why is `x[-which(x > 0)]` not the same as `x[x <= 0]`? 
    ...
    <div class="answer">
    The two expression will give different answers only in the presence of missing values, where `x[x <= 0]` will transform the original `NaN` value to `NA`.
    ...
    This happens because the expression `NaN > 0` evaluates to `NA`. If there are `NA`s in the vector used for logical indexing, then an `NA` is returned in the correspondent element of the vector.
    
    ```{r}
    x <= 0
    
    x[c(NA, rep(TRUE, length(x) - 1))]
    ```
    
    `which()`, on the other hand, returns the position of the elements that meet a specific condition, and so, the original `NaN` value is preserved.
    
    It is good to take note of this behaviour as we often rely on logical indexing for subsetting vectors.
...
1.  What happens when you subset with a positive integer that's bigger
    ...
    x <- c(hello = "Hello", world = "World!")
    x["helo"]
    x[["helo"]]
    x[["hel", exact = FALSE]] # allow partial matching
    ```
    </div>
...
1.  What happens if you subset a tibble as if you're subsetting a list?
    ...
    Subsetting works in the same way as a list.
    
    Much as regular data frames, a tibble is a collection of lists. Unlike lists, they can be indexed similarly to matrices `x[i, j]` `x[[i,j]]`. Moreover, they require all elements in a column to have the same type (list-columns are an exception), and all columns to have the same length.
    
    Here are some examples of subsetting operations for tibble. For further examples and a more in-depth discussion, particularly regarding the differences between tibbles and data frames, see `?tibble::subsetting` and `?tibble::invariants` (for now, [only available in the development version of the package](https://tibble.tidyverse.org/dev/articles/invariants.html).
...
1.  Try and make a tibble that has columns with different lengths. What
    ...
    <div class="answer">
    ```{r, error=TRUE}
    tibble(number = 1:5, letter = LETTERS)
    ```
    </div>
...
1.  Based on the definition above, is it ok to have a list as a
    ...
    <div class="answer">
    It is possible to have a list as a column of a tibble as long as it has the same number of elements as other columns.
