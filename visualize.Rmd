...
1.  What does the `drv` variable describe?  Read the help for `?mpg` to find
    ...
    ::: answer
    The `drv` variable describes the type of drive: font-wheel drive, rear-wheel drive or four-wheel drive.
    :::
...
1.  What happens if you make a scatterplot of `class` vs `drv`? Why is
    ...
    ::: answer
    The plot is not useful because it only shows the `class`/`drive` pairs that are available in the data, but gives no information about their relative frequency.
    ```{r}
...
1.  What's gone wrong with this code? Why are the points not blue?
    ...
    ::: answer
    `colour` needs to be set outside of `aes()`.
    ```{r}
    ggplot(data = mpg) + 
      geom_point(mapping = aes(x = displ, y = hwy), color = "blue")
    ```
...
1.  Map a continuous variable to `color`, `size`, and `shape`. How do
    ...
    ::: answer
    `colour` returns a discrete or continuous palette with respect to the type variable.
    ...
    `shape` returns a warning when it is applied to a categorical variable.
    ```{r, out.width = "50%", fig.align = "default"}
    # left
    ggplot(data = mpg) + 
    ...
    `shape` returns an error when we supply a continous variable. Like before, only six groups are plotted.
    ```{r, error = TRUE}
    # no output
    ggplot(data = mpg) + 
...
1.  What happens if you map the same variable to multiple aesthetics?
    ...
    ::: answer
    The variable is used in all the aesthetics.
    ```{r}
    ggplot(data = mpg) +
...
1.  What does the `stroke` aesthetic do? What shapes does it work with?
    ...
    ::: answer
    The `stroke` aesthetic modifies the width of the border for the shapes (0-14) and (21-24) shown in Figure \@ref(fig:shapes).
    ```{r, out.width = "50%", fig.align = "default"}
    # left: default stroke
    ggplot(data = mpg) +
      geom_point(mapping = aes(x = displ, y = hwy), shape = 23)
    
    # right: modified stroke
    ggplot(data = mpg) +
      geom_point(mapping = aes(x = displ, y = hwy), shape = 23, stroke = 3)
    ```
    :::
...
1.  What happens if you map an aesthetic to something other than a variable 
    ...
    ::: answer
    It creates a dummy variable that is passed to the aesthetic.
    ```{r}
    ggplot(data = mpg) +
...
1.  What happens if you facet on a continuous variable?
    ...
    ::: answer
    ggplot2 will carry out the operation, but we are most likely going to end up with a plot that is not very inforamative because the data is spread out into too many categories. We also lose valuable information on the *level* of the facetting variable.
    ```{r}
    ggplot(data = mpg) +
      geom_point(mapping = aes(x = displ, y = hwy)) +
      facet_wrap(~ cty)
...
1.  What do the empty cells in plot with `facet_grid(drv ~ cyl)` mean?
    ...
    ```{r pairs-drv-cyl, eval = FALSE}
    ggplot(data = mpg) + 
      geom_point(mapping = aes(x = drv, y = cyl))
    ```
    
    ::: answer
    The empty cells mean that certain combinations of `drv` and `cyl` are not available in the data. This particular piece of information can be inferred from the plot below.
    ```{r, ref.label = "pairs-drv-cyl", eval = TRUE}
    ```
    :::

1.  What plots does the following code make? What does `.` do?

    ```{r grid-dot, eval = FALSE}
    ggplot(data = mpg) + 
      geom_point(mapping = aes(x = displ, y = hwy)) +
      facet_grid(drv ~ .)
    ...
    ::: answer
    The first piece of code creates three facets with respect to the values of `drv` and places them as rows in the grid. The second plot organises the facets as columns instead. The `.` serves to indicate whether to let go of the rows or columns in our grid.
    
    ```{r, ref.label = "grid-dot", echo = FALSE, out.width = "50%", fig.align = "default", fig.asp = 1}
    ```
    :::

1.  Take the first faceted plot in this section:

    ```{r facet-class, eval = FALSE}
    ggplot(data = mpg) + 
      geom_point(mapping = aes(x = displ, y = hwy)) + 
      facet_wrap(~ class, nrow = 2)
    ```
    ...
    ::: answer
    Facetting makes it easier to see the underlying trends in each group.
    ```{r, ref.label = "facet-class", eval = TRUE}
    ```
    
    The disadvantage is that it makes it harder to see general trends in the data.
...
1.  Read `?facet_wrap`. What does `nrow` do? What does `ncol` do? What other
    ...
    ::: answer
    `nrow` and `ncol` define the number of rows or columns of the grid.
    
    `facet_grid()` does not have these arguments because the number of columns and rows is determined by the number of unique values of the variables specified by the user.
    :::
...
1.  When using `facet_grid()` you should usually put the variable with more
    ...
    ::: answer
    Figures generally have more horizontal than vertical space, so it makes sense to put the variable with more unique levels in the columns.
    ```{r, out.width = "50%", fig.align = "default"}
    # left, better
    ggplot(data = mpg) + 
      geom_point(mapping = aes(x = displ, y = hwy)) +
...
1.  Run this code in your head and predict what the output will look like.
    ...
    ```{r scatter-color-smooth, eval = FALSE}
    ggplot(data = mpg, mapping = aes(x = displ, y = hwy, color = drv)) + 
      geom_point() + 
    ...
    ::: answer
    ```{r, ref.label = "scatter-color-smooth", eval = TRUE, echo = FALSE, message = FALSE}
    ```
    :::
...
1.  What does `show.legend = FALSE` do?  What happens if you remove it?  
    ...
    ::: answer
    `show.legend = FALSE` removes the legend for a given geom layer. For example, the code below removes the legend for the coloured dots, but not for the coloured lines.
    ```{r, message = FALSE}
    ggplot(data = mpg, mapping = aes(x = displ, y = hwy, color = drv)) + 
    ...
    To remove it everywhere we need to provide the argument in every geom.
    ```{r, message = FALSE}
    ggplot(data = mpg, mapping = aes(x = displ, y = hwy, color = drv)) + 
      geom_point(show.legend = FALSE) + 
    ...
    It is not possible to pass the option globally via `ggplot()`.
...
1.  What does the `se` argument to `geom_smooth()` do?
    ...
    ::: answer
    It removes the grey area around the trend line that represents the confidence interval.
    :::


1.  Will these two graphs look different? Why/why not?

    ```{r samesies, eval = FALSE}
    ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
      geom_point() + 
      geom_smooth()
    ...
    ::: answer
    They will look exactly the same because they share the same data and aesthetic mappings in every layer.
    ```{r ref.label = "samesies", eval =TRUE, out.width = "50%", fig.align = "default", message = FALSE}
    ```
    :::
...
1.  Recreate the R code necessary to generate the following graphs.
    ...
    ::: answer
    The key to prevent a differtiated line colours and a legend is to pass `drv` to the `group` aesthetic.
    ...
    In the second plot the line is determined by the entire data.
    ```{r out.width = "50%", fig.align = "default", message = FALSE}
    p +
      geom_point(aes(colour = drv)) +
      geom_smooth(aes(colour = drv), se = FALSE)
    ...
    In the first plot the `linetype` is determined by `drv`. This is reflected in the legend. The second plot is made by superposing two point layers.
    ```{r out.width = "50%", fig.align = "default", message = FALSE}
    p +
      geom_point(aes(colour = drv)) +
      geom_smooth(aes(linetype = drv), se = FALSE)
...
1.  You might want to override the default mapping from transformed variables
    ...
    ::: answer
    One of those weird behaviours of ggplot2. Something needs to be passed to `group` for the plot to work, but it doesn't necessarily have to be `1`. 
    ```{r}
    ggplot(data = diamonds) + 
      geom_bar(mapping = aes(x = cut, y = stat(prop)))
    
    ggplot(data = diamonds) + 
      geom_bar(mapping = aes(x = cut, y = stat(prop), group = "whatevs"))
    ```
    :::
...
1.  What is the default geom associated with `stat_summary()`? How could
    ...
    ::: answer
    The default geom associated with `stat_summary()` is `geom_pointrange()`. Note however that the default of `geom_pointrange()` is not `stat = "summary"` but `stat = "identity"`, which means that we have to provide the argument explicitely as well as the summary functions.
    ```{r}
    ggplot(data = diamonds) +
      geom_pointrange(
        aes(x = cut, y = depth), 
...
1.  What variables does `stat_smooth()` compute? What parameters control
    ...
    - `y`: predicted value
    - `ymin` and `ymax`: lower and upper confidence interval around the mean
    - `se`: standard error
    
    Its parameters include: `method`, `formula`, `n`, `span`, `level` and `method.args`.
    :::
...
1.  In our proportion bar chart, we need to set `group = 1`. Why? In other
    ...
    ```{r ref.label = "prop", eval = TRUE, echo = FALSE, out.width = "50%", fig.align = "default"}
    ```
  
    Now, the reason why the plots look that way is because they are mapping the proportions *within each group* ([Fair in Fair is 100%](https://stackoverflow.com/a/39879232)) to the `y` aesthetic, instead of the proportion across all groups.
    
    I still don't understand why this happens or why the behaviour is overriden whent the `group` argument is supplied.
    ...
    Even more mysterious is the fact that the plot above needs the `group` aesthetic to work, but the one below doesn't.
    ```{r}
    ggplot(data = diamonds) + 
      geom_bar(
        mapping = aes(
          x = cut, 
          fill = color, 
          y = stat(count / sum(count)) # or ..count.. / sum(..count..)
          )
        )
    ```
    
    Note that there are two ways to refer to the calculated aesthetics produced by the statistic: either by calling the `stat()` function, or by surrounding the variable name with `..`..
...
1.  Compare and contrast `geom_jitter()` with `geom_count()`.
    ...
    ::: answer
    `geom_count()` counts the number of observations at each location and maps the total to the point area.
    ```{r}
    ggplot(data = mpg, mapping = aes(x = cty, y = hwy)) + 
      geom_count()
    ```
...
1.  What's the default position adjustment for `geom_boxplot()`? Create
    ...
    ```{r}
    ggplot(data = mpg, mapping = aes(x = class, y = hwy, colour = drv)) +
      geom_boxplot()
    ```
    :::
...
1.  Turn a stacked bar chart into a pie chart using `coord_polar()`.
    ...
    ::: answer
    Note that we have to specify `theta = "y"` so that the angle is mapped to the `y` variable.
    ```{r}
    ggplot(data = mpg) +
...
1.  What does `labs()` do? Read the documentation.
    ...
    ::: answer
    `labs()` modifies axis, legend and plot lables.
    :::

1.  What's the difference between `coord_quickmap()` and `coord_map()`?

    ::: answer
    `coord_quickmap()` is a quick approximation to `coord_map()`, which projects a portion of the Earth onto a flat 2D plane. Unlike `coord_map()`, `coord_quickmap()` preserves straight lines, which is less realistic.
    :::
...
1.  What does the plot below tell you about the relationship between city
    ...
    ::: answer
    The plot tells me that the highway mpg is always greater than the city highway mpg.
    
    `geom_abline()` adds diagonal with intercept in 0 and slope of 1. `coord_fixed()` ensures that the line is truly at 45° from both axes.
    :::
