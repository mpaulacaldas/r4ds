...
1.  Write for loops to:
    ...
    1. Determine the type of each column in `nycflights13::flights`.
        ...
        for (i in seq_along(nycflights13::flights)) {
          flight_types[[i]] <- typeof(nycflights13::flights[[i]])
        }
...
1.  Eliminate the for loop in each of the following examples by taking 
    ...
    ```
      ...
      ```{r, include = FALSE}
      rm(sd) # otherwise the first code chunk in the map section fails
      ```
...
1.  What happens if you use `for (nm in names(x))` and `x` has no names?
    ...
    ::: answer
    If `x` has no names, the loop will not affect the original object, but no warning or message will be raised.
    ...
    When only some of the elements are named, an error is raised. Note that the named elements will be affected by the operations defined inside the loop.
    ...
    Dangerous things can happen if names are not unique. In the example below, the loop goes through each element in `names(x)` but every time in encounters the duplicate name `"two"`, it takes the value of the first element in the vector that matches that name (i.e. the second element in the vector), applies the transformation (squaring) and replaces it. Since the name is repeated three times, the second element of the original vector ends up being squared three times. The third and fourth elements, which have the duplicated names, remain unchanged.
    
    ```{r, error = TRUE}
    x <- 1:5
    names(x) <- c("one", "two", "two", "two", "five")
...
1.  Write a function that prints the mean of each numeric column in a data 
    ...
    show_mean <- function(df) {
      ...
      max_len_name <- max(str_length(names(df)))
      ...
      for (nm in names(df)) {
        if (is.numeric(df[[nm]])) {
          rounded_mean <- formatC(mean(df[[nm]]), digits = 2, format = "f")
          padded_name <- str_pad(
            str_c(nm, ":"), 
            width = max_len_name + 1, 
            side = "right"
            )
          
          cat(padded_name, rounded_mean, "\n")
...
1.  What does this code do? How does it work?
    ...
    ::: answer
    `trans` is a list populated by two functions, `disp()` and `am()`, which are named after variables from `mtcars`. The loop applies these transformations to the corresponding variables from `mtcars`. It is equivalent to:
    ...
    mtcars[["am"]] <- am(mtcars[["am"]])
    mtcars[["disp"]] <- disp(mtcars[["disp"]])
    ```
...
1.  Read the documentation for `apply()`. In the 2d case, what two for loops
    ...
    ::: answer
    `apply()` applies a function to the rows or columns of a matrix depending on the argument provided to the `MARGIN` argument.
    
    ```{r}
    (mtcars2 <- mtcars[1:5, 1:5])
    
    apply(mtcars2, 1, mean) # rows
    apply(mtcars2, 2, mean) # columns
    ```
    
    If the `MARGIN` argument is set to `c(1, 2)`, then the function will be applied to all elements of the matrix.
    
    ```{r}
    apply(mtcars2, c(1, 2), function(x) (x / 2))
    ```
    
    This second case generalises a loop that iterates over the rows and columns.
    
    ```{r, error= TRUE }
    for (j in seq_along(mtcars2)) {
      for (i in seq_len(nrow(mtcars2))) {
        mtcars2[i, j] <- (mtcars2[i, j] / 2)
      }
    }
...
1.  Write code that uses one of the map functions to:
    ...
    1. Generate 10 random normals from distributions with means of -10, 0, 10, and 100.
        ::: answer
        ```{r, error=TRUE}
        map(c(-10, 0, 10, 100), ~rnorm(10, mean = .x))
        ```
        :::
...
1.  How can you create a single vector that for each column in a data frame
    ...
    ::: answer
    ```{r}
    flag_factors <- function(df) { map_lgl(df, is.factor) }
    flag_factors(iris)
    ```
    :::
...
1.  What happens when you use the map functions on vectors that aren't lists?
    ...
    ::: answer
    `map()` will apply the function to each element of the input vector in the same way it would if the input were a list, but the output will always be a list.
    ```{r}
    map(1:5, runif)
    ```
    :::
...
1.  What does `map(-2:2, rnorm, n = 5)` do? Why?
    ...
    ::: answer
    `map()` takes three arguments, `.x`, `.f` and `...`. To every element of `.x`, `map()` will apply the function defined in `.f`. Behind the scenes, `map()` is making a crucial assumption: it assumes that the elements of `.x` correspond to the first argument of the `.f` function. 
    
    The example below shows one way to overrule this default behaviour. The first argument to `rnorm()`, `n`, is provided explicitely in the `...` argument to `map()`. When this happens, `map()` will assume that the elements of `.x` should be passed on to the second argument of `.f`. In this case, the `mean` argument to `rnorm()`.
    
    The first expression therefore returns a list of random normals of 5 observations for the vector of means `-2:2`.
    ```{r}
    map(-2:2, rnorm, n = 5)
    ```
    ...
    ```{r, eval = FALSE}
    map(-2:2, ~rnorm(n = 5, mean = .x))
    ```
    
    A call to `map_dbl()` fails because the map family of functions will only return vectors of the *same length as the input*. Since `rnorm(n = 5, .x)` returns a vector of length 5 for each element of `.x`, the resulting vector would have to be 5 times the size of the original input vector, which breaks this cardinal rule.
...
1.  Implement your own version of `every()` using a for loop. Compare it with
    ...
    At first glance, `purrr::every()` does a couple of things better:
    - It breaks early and returns `FALSE` if any of the elements is `FALSE`. My version waits until all elements have been evaluated. Same goes for `NA` values.
    - It passes `...` to the predicate function. I did not take this into account.
...
1.  Create an enhanced `col_summary()` that applies a summary function to every
    ...
    col_summary <- function(df, f) {
      map_dbl(keep(df, is.numeric), f)
    }
    
    col_summary(iris, max)
    ```
    :::
...
1.  A possible base R equivalent of `col_summary()` is:
    ...
    ::: answer
    First, let's evaluate the code provided above.
    ```{r, ref.label = "bugs", eval = TRUE, error = TRUE}
    ```
    
    `col_sum3(df[1:2], mean)` and `col_sum3(df[1], mean)` return what is expected: they give the mean of the selected columns. `col_sum3(df[0], mean)` fails because `sapply()` returns an empty list when we evaluate `is.numeric()`. This in turn causes an error in `df[, is_num]`, because we can't subset a data frame with a list.
    
    ```{r}
    sapply(df[0], is.numeric)
    ```
    
    Replacing the `sapply()` calls with map functions gives more predictable outcomes.
