...
1.  Why is `TRUE` not a parameter to `rescale01()`? What would happen if
    ...
    <div class="answer">
    `TRUE` is a parameter to `rescale01()` because we want to prevent the missing values in the input from "spreading" and affecting the calculations for the entire vector.
    
    ```{r}
    rescale01 <- function(x, na.rm = TRUE, finite = FALSE) {
    ...
    This contagion happens because, in the presence of `NA`s, `range()` will return a vector made up of only missing values...
        
    ```{r}
    range(x)
    ```
    
    ...which will in turn affect the last rescaling operation inside the function.
    ...
    Note that here we assume that `finite = FALSE`. If `finite = TRUE`, `NA`s will *always* be removed, regardless of the parameter value of `na.rm`.
        
    ```{r}
    range(x, finite = TRUE)
    range(x, na.rm = FALSE, finite = TRUE)
    ```
...
1.  Practice turning the following code snippets into functions. Think about 
    ...
    prop_missing(c(1:4, NA, NA))
    
    share_of_x <- function(x) { x / sum(x, na.rm = TRUE) }
    share_of_x(1:5)
    
    sd_to_mean <- function(x) { sd(x, na.rm = TRUE) / mean(x, na.rm = TRUE) }
    sd_to_mean(c(1, 7, 3, 5))
    ```
...
1.  write your own functions to compute the variance and skewness of a numeric vector.
    ...
    variance <- function(x) {
      denom <- length(x) - 1
      diff2 <- (x - mean(x, na.rm = TRUE)) ^ 2
      sum(diff2, na.rm = TRUE) / denom
    }
    
    skewness <- function(x) {
      denom  <- length(x) - 2
      diff3  <- (x - mean(x, na.rm = TRUE)) ^ 3
      var3_2 <- var(x, na.rm = TRUE) ^ (3/2)
      sum(diff3, na.rm = TRUE) / (denom * var3_2)
    }
...
1.  Write `both_na()`, a function that takes two vectors of the same length 
    ...
    both_na(c(1, NA, 3, NA, 5), c(1, 2, 3, NA, NA))
    both_na(c(1, NA, 3, NA, 5), c(1, NA, 3, NA, NA))
    ```
...
1.  What do the following functions do? Why are they useful even though they
    ...
    <div class="answer">
    `is_directory()` returns `TRUE` if a path is a directory. The function is useful because it simplifies an operation that we are likely to use a lot in some cases (e.g. when writing files). 
    
    `is_readable()` returns `TRUE` if a file is readable. The biggest advantage to using this function comes from its clarity: we understand immediately what the function does from its name, without having to open up the documentation for `file.access()`. Moreover, unlike `file.access()`, which returns a `0` for success and `-1` for failure, `is_readable()` returns a logical, which makes it easier to write conditions.
    </div>
...
1.  Read the source code for each of the following three functions, puzzle out
    ...
    match_length <- function(y, target) {
      rep(y, length.out = length(target))
    }
    match_length(3, target = letters[1:7])
    ```
    </div>
...
1.  Compare and contrast `rnorm()` and `MASS::mvrnorm()`. How could you make
    ...
    I would change the name of the arguments. Here are my suggestions:
    
    | `rnorm()` | `MASS::mvrnorm()` | Proposed name for argument | Reason |
    |-----------|-------------------|-------------|-----------------------|
    | `n`       |                   | `n_obs`     | Avoids confusion, more informative. |
    |           | `n`               | `n_samples` | Avoids confusion, more informative. |
    | `mean`    | `mu`              | `means`     | Standardises the term across functions. |
...
1.  Make a case for why `norm_r()`, `norm_d()` etc would be better than
    ...
    <div class="answer">
    **Advantages:** The `norm_` prefix highlights the fact that these functions provide information on the normal distribution. Moreover, the consistent prefix makes it easier to search for the functions with RStudio's IDE autocompletion. 
    
    **Disadvantages:** Changing the names of the normal distribution functions would break consistency with other functions of statistical distributions (see `?Distributions` for the comprehensive list). For example, `dnorm`, `dunif` and `dgamma` return the density of the normal, uniform and gamma distribution respectively. A regular user of these functions may find it more useful to follow this convention, which highlights the output of the function. Moreover, searching via tab autocompletion is still relatively straightforward if we prefix the function with the package name (e.g. `stats::d`.)
    </div>
...
if (c(FALSE, FALSE) || c(TRUE, FALSE)) { print("Success!") }

# Evaluates TRUE & TRUE
if (c(TRUE, FALSE) && c(TRUE, FALSE)) { print("Success!") }
```
...
1.  What's the difference between `if` and `ifelse()`? Carefully read the help
    ...
    ***`if (cond) cons`***
      - `cond` cannot be `NA`
      - `cond` can a value of length greater than one, but a warning is raised since only the first element is used.
      - Returns the value of the expression evaluated or `NULL` invisibly if none was specified.
      
    <br/>
    ***`ifelse(test, yes, no)`***
      - `NA`s in `test` will appear in the result.
      - Returns a vector of the same length as `test`.
      - Recycles `yes` or `no` if they are too short.
      - Slower than `if` when `length(test) == 1`
      - Stips attributes (like dates)
    
    <br/>
    **Example 1:** The length of output is determined by first argument in `ifelse()`
    ```{r}
    if (TRUE) { 1:5 } else { 5:10 }
    ifelse(TRUE, 1:5, 5:10)
    ```
    
    **Example 2:** `if()` only evaluates the first element of the condition, `ifelse()` is vectorised.
    ```{r}
    if (c(TRUE, FALSE)) { 1:5 } else { 5:10 }
    ifelse(c(TRUE, FALSE), 1:5, 5:10)
    ```
    
    **Example 3:** `if()` returns an error if the condition is `NA`, `ifelse()` returns `NA`s in the position of the original `NA`s in `test`.
    ```{r, error = TRUE}
    if (NA) {1:5} else {5:10}
    ifelse(NA, 1:5, 5:10)
    ifelse(c(NA, TRUE), 1:5, 5:10)
    ```
...
1.  Implement a `fizzbuzz` function. It takes a single number as input. If
    ...
    <div class="answer">
    I organise my conditions in terms of strictness and take advantage of `return()` to return the result early if the condition is met.
...
1.  How could you use `cut()` to simplify this set of nested if-else statements?
    ...
    As we can see below, the problem with the if-else statements is that they require `temp` to be an atomic vector. When it is not, the `if` statements use only the first element of the vector to evaluate the condition, therefore returning only one output. `cut()`, on the other hand, is *vectorised*, which means that it will evaluate the conditions for each element of the input vector and will return an output of equal size.
...
1.  What happens if you use `switch()` with numeric values?
    ...
    <div class="answer">
    If you provide `switch()` with a numeric value *n*, then the *n + 1* element of `switch()` is returned.
    ```{r}
    one_to_five <- function(x) {
      switch(x, "one", "two", "three", "four", "five")
    }
...
1.  What does this `switch()` call do? What happens if `x` is "e"?
    ...
    When there is a match to an alternative with a missing element (like `a` and `c` below), then `switch()` will return the next non-missing element.
    
    ```{r}
    weird_switch <- function(x) {
      switch(x, 
        a = ,
        b = "ab",
        c = ,
    ...
    ...*unless* an unnamed element has been provided to `switch()` that will act as a "default" value.
    
    ```{r}
    default_switch <- function(x) {
      switch(x, 
        a = ,
        b = "ab",
        c = ,
...
1.  What does `commas(letters, collapse = "-")` do? Why?
    ...
    <div class="answer">
    This call returns an error because the `collapse` arguement to `stringr::str_c()` was already provided in the body of the `commas()` function.
    ```{r, error = TRUE}
    commas(letters, collapse = "-")
    ```
    
    If we want to allow the user to change the value of `collapse`, then we have to add it as an argument to `commas()`
    ```{r}
    commas <- function(..., collapse = ", ") stringr::str_c(..., collapse = collapse)
    
    # default is still a comma
    commas(letters)
    
    # but the user can overwrite it
    commas(letters, collapse = "-")
    ```
...
1.  It'd be nice if you could supply multiple characters to the `pad` argument, 
    ...
    <div class="answer">
    The `rule()` function implicitely assumes that `pad` is a character vector of length one. Therefore, when we supply it with a character like `"-+"`, the length of the pad will be doubled.
    
    ```{r}
    rule("Important output", pad = "-+")
    ```
    
    To fix it, we need to make sure that the `times` argument to `stringr::str_dup()` is divided by the length of `pad`. I use integer division to make sure that the resulting character is always of the same length.
    
    ```{r}
    rule <- function(..., pad = "-") {
      title     <- paste0(...)
      width     <- getOption("width") - nchar(title) - 5
      width_pad <- stringr::str_length(pad)
      times     <- (width %/% width_pad) + (width %% width_pad)
      cat(title, " ", stringr::str_dup(pad, times), "\n", sep = "")
    }
...
1.  What does the `trim` argument to `mean()` do? When might you use it?
    ...
    <div class="answer">
    `trim` removes up to 50% of the observations at the tails of `x`. This specification may be useful when we want to exclude outliers from our calculation.
...
1.  The default value for the `method` argument to `cor()` is 
    ...
    <div class="answer">
    This means that only the values `"pearson"`, `"kendall"` or `"spearman"` can be provided to `method`. The default is `"pearson"`. 
    
    Note that the values *can* be abbreviated. This is allowed because `cor()` calls `match.arg()` under the hood. For more details on `match.arg()`, I recommend this blog post: <https://alistaire.rbind.io/blog/match.arg/>.
    </div>
